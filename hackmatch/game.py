...
RawSettings: 't.TypeAlias' = t.Dict[str, str]
...
def check_settings(data: t.Optional[c.GameSettings] = None, raise_on_error=False) -> bool:
    if data is None:
        data = read_settings()
    ...
    log.warning("Incorrect game settings: \n\t%s",
                "\n\t".join((f"{k}: {v1}, expected {v2}"
